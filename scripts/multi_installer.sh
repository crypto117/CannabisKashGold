...
# CannabisKash Multi-installer
# a one line clone-and-compile for cannabiskashgold:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/chronickash/cannabiskash/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/chronickash ]; then
            read -r -p "${1:-cannabiskash directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old cannabiskash directory" && echo
        ...
        git clone -b master -q https://github.com/chronickash/chronickash chronickash   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd ChronicKash
    fi
}
...
_note "Look in 'cannabiskash/build/src/' for the executible binaries. See 'https://github.com/ChronicKash/CannabisKashGold' for more project support. Cowabunga!"
